...
export const SitePagesMenu: React.FC<SitePagesMenuProps> = ({
  ...
  return (
    <>
      ...
      >
        {/* Header Title */}
        <div className="px-4 pb-2.5 mb-2 border-b border-stone-200 flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-amber-900 flex items-center gap-1.5 font-serif">
            <Sparkles className="w-3.5 h-3.5 text-amber-600 fill-amber-500" />
            {isAr ? 'صفحات وأقسام الموقع' : 'Site Pages & Navigation'}
          </span>
          <span className="text-[10px] text-stone-700 font-mono font-bold">
            xn--mgblao3hjb.store
          </span>
        </div>
        ...
        <div className="px-2 space-y-1">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-stone-900 hover:bg-amber-50 hover:text-amber-950 transition-colors text-start group cursor-pointer"
          >
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-amber-100 text-amber-900 flex items-center justify-center group-hover:bg-amber-200 transition-colors">
                <Store className="w-4 h-4" />
              </div>
              <div>
                <span className="font-bold text-sm block text-stone-950">
            ...
            <ArrowUpRight className="w-3.5 h-3.5 text-stone-600 group-hover:text-amber-800" />
          </button>

          {/* 2. أقسام الموقع (معروضة تحت بعضها بالكامل دون إخفاء) */}
          <div className="pt-1.5 pb-1">
            <div className="px-3 py-1 text-[11px] font-bold text-stone-700 uppercase tracking-wider flex items-center gap-1.5">
              <Layers className="w-3.5 h-3.5 text-amber-700" />
              <span>{isAr ? 'أقسام المتجر الرئيسية:' : 'Store Departments:'}</span>
            </div>

            <div className="space-y-0.5 ps-2 pt-1 border-s-2 border-amber-300 ms-3">
              {CATEGORIES.filter(c => c.id !== 'all').map((cat) => (
                <button
                  key={cat.id}
                  id={`menu-category-${cat.id}`}
                  onClick={() => handleCategoryClick(cat.id)}
                  className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs font-bold text-stone-800 hover:bg-stone-100 hover:text-amber-950 transition-colors text-start cursor-pointer group"
                >
                  <span className="truncate">
                    {isAr ? cat.nameAr : cat.nameEn}
                  </span>
                  {cat.badgeAr && (
                    <span className="text-[9px] bg-amber-100 text-amber-950 font-black px-1.5 py-0.5 rounded border border-amber-300">
          ...
          <div className="pt-2 pb-1 bg-gradient-to-br from-amber-500/10 via-amber-50 to-orange-50/50 rounded-xl p-2.5 border border-amber-300/80 my-2 space-y-1.5 shadow-xs">
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-2.5 py-2 rounded-lg bg-white hover:bg-emerald-50 text-stone-900 transition-colors text-start shadow-xs border border-emerald-300 cursor-pointer group"
            >
              <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-stone-900 hover:bg-emerald-50 hover:text-emerald-950 transition-colors text-start group cursor-pointer"
          >
            <div className="flex items-center gap-2.5">
            ...
            <ArrowUpRight className="w-3.5 h-3.5 text-stone-600 group-hover:text-emerald-700" />
          </button>
